...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            print('form is valid')
            form.save()#INSERT INTO CustomUser Values (whatever is in form())
            username = form.cleaned_data.get('username')
            temp = CustomUser.objects.get(username = username)# SELECT * FROM CustomUser WHERE username = username;

            if form.cleaned_data.get('user_type') == 'ST':
                
                if form2.is_valid():
                    temp_student = Student(user = temp, teacher = form2.cleaned_data.get('teacher'))#INSERT INTO Student (user,teacher) values(temp,form2)
                    temp_student.save()
...
def profile(request):
    if not request.user.is_authenticated:
        ...
        if user_type == 'ST':
            student = Student.objects.get(user = user)# SELECT * FROM CustomUser WHERE user.id = request.user.id
            ...
            return render(request,'Profiles/tutorProfile_tutor.1.html')
    ...
    if not request.user.is_authenticated:
        ...
        if user_type == 'ST':
            ...
            if Module.objects.all():
                new_modules = Module.objects.order_by('-pub_date')[:3]# SELECT * FROM Module ORDER BY pub_date DESC LIMIT 3
                latest_module = Module.objects.order_by('-pub_date').filter()[0]#SELECT * FROM Module ORDER BY pub_date DESC LIMIT 1
        ...
        if request.method == 'POST':
            ...
            if user_update_form.is_valid():
                ...
                if  user_update_form.cleaned_data.get('street_name') == "":
                    ...
                    current_user.street_name = user_update_form.cleaned_data.get('street_name') #UPDATE FROM current_user SET street_name = user_update_form.cleaned_data.get('street_name'); 
...
def student_list(request):
    if not request.user.is_authenticated or request.user.user_type == "ST":
        ...
        students =  Student.objects.filter(teacher = request.user )# SELECT * FROM Student WHERE teacher = request.user;
...
def module_tutor(request,ModuleTitle):
    if not request.user.is_authenticated or request.user.user_type == "ST":
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() and "student" in request.POST:
                print('I GOT HERE')
                student = request.POST['student']
                homeworkdetail = request.POST['HomeworkDetail']
                Sub = HomeworkSubmission.objects.get(StudentID_id = student, id = homeworkdetail)# SELECT * FROM HomeworkSubmission WHERE StudentID_id = student, id = homeworkdetail)
                Sub.Comment = form.cleaned_data.get('Comment') #UPDATE FROM Sub Alter Comment = form.cleaned_data.get('comment')
                Sub.save()
    ...
    if not request.user.is_authenticated or request.user.user_type == "ST":
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                module = Module(ModuleTitle = ModuleTitle, Description = Description, Tutor = Tutor, file = file)# INSERT INTO Module (ModuleTitle, Description,Tutor,file) VALUES 
                module.save() 
                print('form is valid')
